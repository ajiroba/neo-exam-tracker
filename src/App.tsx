...
export default function NEOTracker() {
    ...
    try {
      ...
      if (json.success) {
        const parsed = json.data.map(e => ({
          ...e,
          examScore: e.examScore === "" || e.examScore === null ? null : Number(e.examScore),
          attempts: Number(e.attempts) || 0,
          id: Number(e.id),
        }));
        setEmployees(parsed);
      } else {
        setError("Failed to load data: " + json.error);
      }
    } catch (err) {
  ...
  // ── Save employee update to Google Sheets ──────────────────
  ...
  const addEmployee = async () => {
    ...
    const emp = { id: newId, ...newEmp, examDate: null, examScore: null, attempts: 0, status: "unscheduled" };
    setSaving(true);
    try {
      await fetch(SCRIPT_URL, {
        method: "POST",
  ...
  function handleReschedule(emp) { setRescheduleModal(emp); setRescheduleDate(emp.examDate || ""); }
  ...
  function handleScoreEntry(emp) { setScoreModal(emp); setScoreInput(emp.examScore !== null ? String(emp.examScore) : ""); }
  ...
  if (error && employees.length === 0) return (
    <div style={{ minHeight: "100vh", background: "#0d0f14", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'IBM Plex Mono', monospace", color: "#f87171", padding: 32 }}>
      ...
      <div style={{ marginTop: 16, fontSize: 11, color: "#475569", maxWidth: 480, textAlign: "center", lineHeight: 1.8 }}>
        Make sure you have replaced <span style={{ color: "#a5b4fc" }}>SCRIPT_URL</span> in the code with your deployed Google Apps Script Web App URL.
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0f14", color: "#e2e8f0", fontFamily: "'IBM Plex Mono', 'Courier New', monospace", fontSize: "13px" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pill-btn, .action-btn { transition: all 0.15s ease; cursor: pointer; border: none; outline: none; }
        .pill-btn:hover, .action-btn:hover { filter: brightness(1.2); transform: translateY(-1px); }
      ...
      <div style={{ borderBottom: "1px solid #1e2230", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, position: "sticky", top: 0, background: "#0d0f14", zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 17, letterSpacing: "0.02em", color: "#f1f5f9" }}>NEO EXAM TRACKER</div>
            <div style={{ color: "#64748b", fontSize: 11, letterSpacing: "0.08em" }}>NEW EMPLOYEE ORIENTATION · PASS MARK {PASS_MARK}%</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          {saving && <div style={{ color: "#6366f1", fontSize: 11, letterSpacing: "0.06em" }}><span className="spin">⟳</span> SAVING...</div>}
          {lastSaved && !saving && <div style={{ color: "#22c55e", fontSize: 11, letterSpacing: "0.06em" }}>✓ SAVED {lastSaved.toLocaleTimeString()}</div>}
          {stats.critical > 0 && (
      ...
      {/* Tabs */}
      <div style={{ borderBottom: "1px solid #1e2230", padding: "0 32px", display: "flex", gap: 0 }}>
                  ...
                  return (
                    <div key={level} style={{ marginBottom: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ color: s.text, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase" }}>{level}</span>
                      ...
                      <div style={{ height: 6, background: "#1e2230", borderRadius: 3, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${pct}%`, background: s.badge, borderRadius: 3, transition: "width 0.5s ease" }} />
                      </div>
                    </div>
                  );
                })}
              ...
              {enriched.filter(e => e.escalation?.level === "critical" || e.escalation?.level === "high").length === 0 ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", gap: "10px 16px", alignItems: "center" }}>
                  ...
                  {enriched.filter(e => e.escalation?.level === "critical" || e.escalation?.level === "high").map(e => {
                    const s = escalationStyle[e.escalation.level];
                    return [
                      <div key={e.id + "n"} style={{ color: "#e2e8f0", padding: "8px 0" }}>{e.name}</div>,
                      <div key={e.id + "d"} style={{ color: "#64748b" }}>{e.department}</div>,
                      <div key={e.id + "l"}><span style={{ background: s.bg, border: `1px solid ${s.border}`, color: s.text, borderRadius: 4, padding: "2px 8px", fontSize: 10, letterSpacing: "0.08em" }}>{e.escalation.label}</span></div>,
                      <div key={e.id + "dt"} style={{ color: s.text, fontSize: 12 }}>{e.escalation.detail}</div>,
                ...
                return (
                  <div key={e.id} className="row-hover" onClick={() => setSelectedRow(selectedRow === e.id ? null : e.id)}
                    ...
                    <div>
                      {esc ? (
                        <div>
                          <span style={{ background: escStyle.bg, border: `1px solid ${escStyle.border}`, color: escStyle.text, borderRadius: 4, padding: "2px 8px", fontSize: 10, letterSpacing: "0.06em", display: "inline-block" }}>{esc.label}</span>
                    ...
                    <div style={{ display: "flex", gap: 8 }} onClick={ev => ev.stopPropagation()}>
                      <button className="action-btn" onClick={() => handleReschedule(e)} style={{ background: "#1a1d24", border: "1px solid #2d3344", color: "#94a3b8", borderRadius: 6, padding: "5px 10px", fontSize: 11, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace" }}>{e.examDate ? "↻" : "+"} {e.examDate ? "Resched" : "Schedule"}</button>
        ...
        {tab === "calendar" && (
          <div className="fadein">
                      ...
                      <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                        {[{ label: "Passed", val: data.passed, color: "#22c55e" }, { label: "Failed", val: data.failed, color: "#ef4444" }, { label: "Pending", val: data.scheduled - data.passed - data.failed, color: "#6366f1" }].map(s => (
                          <div key={s.label} style={{ flex: 1, textAlign: "center" }}>
                            ...
                            <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.06em" }}>{s.label.toUpperCase()}</div>
                          </div>
                        ))}
                      </div>
                          ...
                          return (
                            <div key={e.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                {esc && e.status !== "passed" && <span style={{ background: escStyle.bg, border: `1px solid ${escStyle.border}`, color: escStyle.text, borderRadius: 3, padding: "1px 6px", fontSize: 9 }}>{esc.label}</span>}
                              </div>
                            </div>
                          );
                        })}
            ...
            <div style={{ background: "#111318", border: "1px solid #1e2230", borderRadius: 12, padding: 24 }}>
              ...
              {enriched.filter(e => { const diff = Math.ceil((e.deadline - today) / (1000 * 60 * 60 * 24)); return diff >= 0 && diff <= 90 && e.status !== "passed"; })
                .sort((a, b) => a.deadline - b.deadline)
                .map(e => {
                  const daysLeft = Math.ceil((e.deadline - today) / (1000 * 60 * 60 * 24));
